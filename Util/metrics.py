...
def evaluate_detections(all_boxes, all_gt_infos, classes,
    ...
    for classIndex, className in enumerate(classes):
        if className != '__background__' and className!='0:__background__':
            BBGT = all_gt_infos[className]
            for BB in BBGT:
                BB['bbox'] = np.asarray(BB['bbox']).astype(np.float32)
                R = BB['det']
    ...
    for classIndex, className in enumerate(classes):
        print ('classIndex,classname:',(classIndex,className))
        if className != '__background__' and className!='0:__background__':
            ...
            aps[className] = ap
            recs[className] = rec
            precs[className] = prec
    
    mAp = np.nanmean(list(aps.values()))
    mRec = np.nanmean(list(recs.values()))
    mPrec = np.nanmean(list(precs.values()))
    print(mAp,mRec,mPrec)
...
def _voc_computePrecisionRecallAp(class_recs, confidence, image_ids, BB, ovthresh=0.5, use_07_metric=False):
    ...
    len(class_recs)=images len(class_recs['bbox']=rois
    '''
    if len(BB) == 0:
        return 0.0, 0.0, 0.0
    ...
    BB = BB[sorted_ind, :]
    image_ids = [image_ids[x] for x in sorted_ind]  #rois*images
